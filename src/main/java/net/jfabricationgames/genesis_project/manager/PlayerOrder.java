...
public class PlayerOrder<T> {
	...
	/**
	 * Start the game with choosing a random player order (and letting all players pass because the current turn is 0).
	 */
	public void startGame(List<T> players) {
		chooseRandomOrder(players);
		while (!isTurnEnd()) {
			T active = getActivePlayer();
			playerPassed(active);
			if (!isTurnEnd()) {
				nextMove();
			}
	...
	/**
	 * Get the order of the players in this turn. If the players have already passed they don't show up in the array.
	 * 
	 ...
	 * 
	 * @return The player order as an array.
	 */
